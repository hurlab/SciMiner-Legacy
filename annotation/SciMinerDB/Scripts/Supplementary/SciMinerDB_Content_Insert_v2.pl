...
#			from SciMinerDB_Content_Extract.pl
#
#	Version 2:  (1) Only the PMIDs not currently in the database are inserted
#				(2) IGNORE check is now omitted. 
#					If you want to apply additional IGNORE filter, 
#					uncomment from the line#54.
#
#############################################################################
...
#  -----------------------------------------------------------------------------
#  Retrieve currently available PMID
my $sql			= 'SELECT pmid FROM document';
my $sth			= $dbh->prepare($sql);
$sth->execute();
my @row         = ();

my %pmid2check	= ();		# existing PMID
...
if ((not defined $ARGV[0]) || (! -d $ARGV[0]))
{	#  Directory existence
...
{	print STDERR "! Inserting data into document table ...\n";
	...
	{	my $line = $_;
		...
		#  Check if pmid is already in the db
		if (defined $pmid2check{$tmpSplit[1]})
		{	print "Skipping $tmpSplit[1]\n";
			next;
		}
	...
	{	print STDERR "! Inserting data into docmesh table ...\n";
		...
		{	my $line = $_;
			...
			#  Check if pmid is already in the db
			if (defined $pmid2check{$tmpSplit[2]})
			{	next;
			}
