...
# Modified by David J. States 1/05/02
#
...
($opt_d, $opt_i, $opt_o, $opt_docid) = @ARGV;

if (defined ($opt_h) || !defined ($opt_i) || !defined ($opt_d) || !defined ($opt_o) || !defined($opt_docid))
{
   $opt_h = 1;
   print "Usage: boundary.pl honorifics_file input_file output_file docid\n";
   exit;
}
...
local $sid = 0;
local $docID = $opt_docid;
my $curSection;
# Main loop to read the input file
while (<INPUT>) {
	$newline = $_;
	chomp($newline);
	chomp($newline);

	# Lines that contain just an XML tag are breaks
	if ($newline =~ m/^\s*<\/?(\S+.*)\/?>\s*$/) {
		$curSection = $1;
		Process($paragraph, $curSection);
	...
	if ($newline =~ /^\s+$/) { # If next line is empty
		...
		$paragraph = "";
		print OUTPUT "\n";
	} else {                   # If next line is not empty append to paragraph
   ...
   for($i=0; $i<@words; $i++) {
	  ...
	  if($words[$i] =~ /<\/[A-Z]+>$/ or 
		...
		$sentence = substr($sentence, 1);
      		...
      		print OUTPUT "$docID\t$curSection\t$sid\t$sentence\n";
		$sid++;
