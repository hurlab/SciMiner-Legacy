...
#******************************************************************************
#
#                generateGeneListFromMeSHHTML.cgi for SciMiner on the web
#
...
my $meshTreeCode		= param("meshTreeCode");
my $meshDesc			= param("meshDesc");
...
if ( ((defined $currentBaseDir) && ($currentBaseDir ne "")) &&
	 ...
	 ((defined $meshDesc) && ($meshDesc ne "")) &&
	 ((defined $meshTreeCode) && ($meshTreeCode ne ""))
   )
{	$pmidListFileName	= "$resultBaseDir/pmidList.html";
	
	#  First check the file to be created
	my $newHTMLToBeCreated	= "$currentBaseDir/MeSH/pmidList_".$meshTreeCode.'_'.$typeColumn.'.txt';
	
	print "<p class='titleBarName1'>$tmpValue documents for $meshDesc ($meshTreeCode) of \'$typeName\' section</p>";
	...
	{	#  Load PMIDs to be extracted
		open (LIST, "$currentBaseDir/MeSHEnrichmentResult.pmid");
		while (<LIST>)
		{	my $line = $_;
			if ($line =~ /^#/)
			{	next;
		...
		if (!$meshIDFound)
		{	push @errorMessage, "No corresponding MeSHTree ID found $meshTreeCode from $resultBaseDir...";
		}else
		{	# Now load the pmidList.html file and recreate a new htmlFild
			open (ORIHTML, "$resultBaseDir/pmidList.html") || return(-1);
			my @content	= <ORIHTML>;
			close ORIHTML;
				...
				{	my $newMatch	= $&;
					...
					{	if ($newMatch	=~ /^<tr>.*?>(\d+)<\/td>/)
						{	if (defined $pmidCheck{$1})
							{	$newMatch	=~ s/"pmid2ContDir/"$resultBaseDir\/pmid2ContDir/;
								$newMatch	=~ s/"Docs/"$resultBaseDir\/Docs/;
								$finalResultString	.= $newMatch."\n";
							}
						}
					}
...
{	
	...
	push @errorMessage, "meshTreeCode $meshTreeCode<br>";
	push @errorMessage, "meshDesc $meshDesc<br>";	
