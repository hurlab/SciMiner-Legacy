...
{	$geneSummaryFileName	= "$resultBaseDir/geneSummary.html";
	
	#  First check the file to be created
	my $newHTMLToBeCreated	= "$currentBaseDir/Pathway/geneSummary_".$pathwayID.'_'.$typeColumn.'.txt';
	...
	{	#  Load PMIDs to be extracted
		open (LIST, "$currentBaseDir/Pathway/SigTest_Pathway.hugoID");
		while (<LIST>)
		{	my $line = $_;
			if ($line =~ /^#/)
			{	next;
		...
		{	# Now load the pmidList.html file and recreate a new htmlFild
			open (ORIHTML, "$resultBaseDir/geneSummary.html") || return(-1);
			my @content	= <ORIHTML>;
			close ORIHTML;
				...
				{	my $newMatch	= $&;
					...
					{	if ($newMatch	=~ /hgnc_id=(\d+)/)
						{	if (defined $hugoIDCheck{$1})
							{	$newMatch	=~ s/"pmid2ListDir/"$resultBaseDir\/pmid2ListDir/;
								$newMatch	=~ s/"hugoID2ContDir/"$resultBaseDir\/hugoID2ContDir/;
								$finalResultString	.= $newMatch."\n";
							}
						}
					}
