...
{	$pmidListFileName	= "FinalResults/$resultBaseDir/pmidList.html";
	...
	my $newHTMLToBeCreated	= "FinalResults/$currentBaseDir/MeSH/pmidList_".$meshTreeCode.'_'.$typeColumn.'.txt';
	...
	{	#  Load PMIDs to be extracted
		open (LIST, "FinalResults/$currentBaseDir/MeSHEnrichmentResult.pmid");
		while (<LIST>)
		{	my $line = $_;
			if ($line =~ /^#/)
			{	next;
