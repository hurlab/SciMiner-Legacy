#!/bin/bash
# SciMiner Infrastructure Setup Script
...
# Stage 1: Check and Install Apache
echo "==================================="
...
# Stage 2: Check and Install MariaDB
echo ""
echo "==================================="
echo "Stage 2: MariaDB Database Setup"
echo "==================================="
...
# Stage 3: Database Configuration Check
echo ""
echo "==================================="
echo "Stage 3: Database Configuration"
echo "==================================="
...
if [[ ${DB_REINSTALLED:-no} == "yes" ]]; then
    ...
    # Check if sciminer database exists (using root access without password, or attempt to connect)
    echo "To check existing database configuration, we need credentials."
    read -p "Do you want to check if sciminer database is already configured? (y=Yes, n=Skip) [y]: " check_db

    if [[ $check_db =~ ^[Yy]$ ]]; then
        echo ""
        print_status "Enter database credentials to check configuration:"
        ...
        if mysql -u $ADMIN_USER -p$ADMIN_PASS -e "SELECT 1;" &>/dev/null; then
            ...
            DB_EXISTS=$(mysql -u $ADMIN_USER -p$ADMIN_PASS -e "SHOW DATABASES LIKE '$DB_NAME';" 2>/dev/null | grep "$DB_NAME" | wc -l)

            if [ $DB_EXISTS -gt 0 ]; then
                # Check if user exists
                USER_EXISTS=$(mysql -u $ADMIN_USER -p$ADMIN_PASS -e "SELECT User FROM mysql.user WHERE User='$DB_USER';" 2>/dev/null | grep "$DB_USER" | wc -l)

                if [ $USER_EXISTS -gt 0 ]; then
                    # Try to test sciminer user with default password
                    if mysql -u $DB_USER -p$DB_PASS -e "USE $DB_NAME; SELECT 1;" &>/dev/null; then
                        ...
                        print_warning "Database and user exist, but cannot authenticate"
                        print_warning "The password for user '$DB_USER' may be different from the default"
                        read -p "Enter correct password for '$DB_USER' user (or press Enter to skip): " USER_PASS
                        if [[ -n $USER_PASS ]]; then
                            if mysql -u $DB_USER -p$USER_PASS -e "USE $DB_NAME; SELECT 1;" &>/dev/null; then
                                table_count=$(mysql -u $DB_USER -p$USER_PASS $DB_NAME -e "SHOW TABLES;" 2>/dev/null | wc -l)
                                if [ $table_count -gt 1 ]; then
                                    print_success "Database '$DB_NAME' is configured with $((table_count-1)) tables"
                                    DB_CONFIGURED="yes"
                                    # Update the password variable for later use
                                    DB_PASS=$USER_PASS
                                else
                                ...
                                print_warning "Cannot authenticate with provided password"
                                DB_CONFIGURED="no"
                            fi
                        else
                            DB_CONFIGURED="partial"
                    ...
                    print_warning "Database '$DB_NAME' exists but user '$DB_USER' does not"
                    DB_CONFIGURED="no"
                fi
            else
            ...
            print_warning "Cannot connect to database with provided admin credentials"
    ...
    # Based on configuration status, decide what to do
    if [[ $DB_CONFIGURED == "yes" ]]; then
        print_success "Database configuration is complete!"
        echo ""
        # Check if SciMiner config file matches database settings
        if [ -f "$SCIMINER_HOME/ANNOTATION/SciMinerDB/annotationENV.ini" ]; then
            print_status "Verifying SciMiner configuration matches..."
            # You could add more verification here if needed
        fi
    else
        echo ""
        print_warning "Database needs to be configured for SciMiner"
        echo ""
        ...
        echo "  - Import schema from sciminer.sql"
        echo "  - Update SciMiner configuration"
        echo ""
...
# Stage 6: Apache Configuration for SciMiner
echo ""
echo "==================================="
echo "Stage 6: Apache Configuration"
echo "==================================="
...
# Stage 7: Final Configuration and Testing
echo ""
echo "==================================="
...
if [ -f "$SCIMINER_HOME/ANNOTATION/SciMinerDB/annotationENV.ini" ]; then
    ...
    cp "$SCIMINER_HOME/ANNOTATION/SciMinerDB/annotationENV.ini" "$SCIMINER_HOME/ANNOTATION/SciMinerDB/annotationENV.ini.backup.$(date +%Y%m%d_%H%M%S)"
...
if [[ ${DB_CONFIGURED:-no} == "yes" ]]; then
    DB_CHECK=$(mysql -u $DB_USER -p$DB_PASS -e "SELECT 'Connected';" 2>/dev/null || echo "Error")
    table_count=$(mysql -u $DB_USER -p$DB_PASS $DB_NAME -e "SHOW TABLES;" 2>/dev/null | wc -l)
...
elif [[ ${DB_CONFIGURED:-no} == "unknown" ]]; then
...
if [[ ${DB_CONFIGURED:-no} != "yes" ]]; then
    ...
    echo "  1. Run Perl module installation: sudo bash $SCIMINER_HOME/INSTALL_PERL_MODULES.sh"
fi
