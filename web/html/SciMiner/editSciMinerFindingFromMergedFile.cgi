...
{	my $sen2geneID					= shift;
	...
	{	#  Check for the remaining file existence
		$sql			= "SELECT sen2geneID, geneID, hgncID, approvedSymbol, matchString, actualString, inExClude, inExCludeCond FROM sentence2gene where sen2geneID in ($targetSen2GeneIDString)";
		$sth         	= $dbh->prepare($sql);
		$sth->execute();
		while(@row = $sth->fetchrow_array)
	...
	{	#  Check for the remaining file existence
		$sql			= "SELECT sen2geneID, geneID, hgncID, approvedSymbol, matchString, actualString, inExClude, inExCludeCond FROM sentence2gene where sen2geneID in ($targetSen2GeneIDString) and pmid = $pmid";
		$sth         	= $dbh->prepare($sql);
		$sth->execute();
		while(@row = $sth->fetchrow_array)
		{	# Check for matching condition
