...
{	$pmidListFileName	= "$resultBaseDir/pmidList.html";
	...
	my $newHTMLToBeCreated	= "$currentBaseDir/MeSH/pmidList_".$meshDescID.'_'.$typeColumn.'.txt';
	my $meshDesc			= '';
	...
	{	#  Load PMIDs to be extracted
		my $pmidFileName	= '';
		if (-f "$currentBaseDir/MeSHCodeDistribution.pmid")
		{	$pmidFileName = "$currentBaseDir/MeSHCodeDistribution.pmid";
		}else
		{	$pmidFileName = "$currentBaseDir/MeSHEnrichmentResult.pmid";
		}
