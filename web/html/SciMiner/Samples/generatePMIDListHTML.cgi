...
#******************************************************************************
#
#                generateGeneListHTML.cgi for SciMiner on the web
#
...
my $extHugoID			= param("extHugoID");
my $hugoSymbol			= param("hugoSymbol");
...
if ( ((defined $currentBaseDir) && ($currentBaseDir ne "")) &&
	 ...
	 ((defined $hugoSymbol) && ($hugoSymbol ne "")) &&
	 ((defined $extHugoID) && ($extHugoID ne ""))
   )
{	$pmidListFileName	= "$resultBaseDir/pmidList.html";
	
	#  First check the file to be created
	my $newHTMLToBeCreated	= "$currentBaseDir/Gene/pmidList_".$extHugoID.'_'.$typeColumn.'.txt';
	
	print "<p class='titleBarName1'>$tmpValue documents for $hugoSymbol ($extHugoID) of \'$typeName\' section</p>";
	...
	{	#  Load PMIDs to be extracted
		open (LIST, "$currentBaseDir/GeneEnrichmentResult.pmid");
		while (<LIST>)
		{	my $line = $_;
			if ($line =~ /^#/)
			{	next;
		...
		if (!$hugoIDFound)
		{	push @errorMessage, "No corresponding HUGO ID found $extHugoID from $resultBaseDir...";
		}else
...
{	
	...
	push @errorMessage, "extHugoID $extHugoID<br>";
	push @errorMessage, "hugoSymbol $hugoSymbol<br>";	
