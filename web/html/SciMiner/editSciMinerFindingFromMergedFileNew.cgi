...
#******************************************************************************
#
#        editSciMinerFindingFromMergedFile.cgi for SciMiner on the web
#
...
#  This version differs in that HUGO ID should also match.
#******************************************************************************
BEGIN {
...
{	if ( ((defined $sen2geneID) && ($sen2geneID ne "")) &&
	...
	{	#  Retrieve sen2geneID from the sen2geneIDfile
		my @targetSen2GeneID			= ();
		...
		{	my ($updateStatus, $updateCount, $message) = update_by_matchString_actualString_scope($sen2geneID, $fileNameBase, $matchString, 
					$actualString, $oldHUGOID, $newHugoID, param("editScope"), \%annoENV, $editLevel, $userID, $pmid, $targetSen2GeneIDString);
			if ($updateStatus ==0)
		...
		{	my ($deleteStatus, $deleteCount) = delete_by_matchString_actualString_scope($sen2geneID, $fileNameBase, $matchString, 
					$actualString, $oldHUGOID, param("editScope"), \%annoENV, $editLevel, $userID, $pmid, $targetSen2GeneIDString);
			if ($deleteStatus ==0)
...
{	print "	<form name=\"form1\" action=\"editSciMinerFindingFromMergedFile.cgi\" method=\"POST\" enctype=\"multipart/form-data\">
			...
			<select name=\"editScope\" size=\"1\" onmouseover=\"Tip('* <b><u>This specific finding</u></b>: Change is only applied to this identification.<br><br>* <b><u>Within this document</u></b>: Change is only applied to the document for any identification <br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;with the exactly same <b>HUGO ID</b>, <b>Match String></b>, and <b>Actual String</b>.<br><br>* <b><u>Within this document - All by the matchString</u></b>: Change is applied to only to the <b>current document (PMID)</b><br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;for any identification has the exactly same <b>HUGO ID</b> and <b>Match String</b>.<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<b>(Actual String is <b>NOT</b> checked.)</b><br><br>* <b><u>Within the query corpus</u></b>: Change is applied to every document in the <b>query</b> for any identification <br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;with the exactly same <b>HUGO ID</b>, <b>Match String</b>, and <b>Actual String</b>.<br><br>* <b><u>Within the query corpus - All by the matchString</u></b>: Change is applied to <b>every document</b> in the <b>query</b><br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;for any identification has the exactly same <b>HUGO ID</b> and <b>Match String</b>.<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<b>(Actual String is <b>NOT</b> checked.)</b><br>')\" onmouseout=\"UnTip()\">
				        <option selected value=\"document\">Within this document</option>
			...
			<a href=\"#\" onclick=\"confirm_update()\" onmouseover=\"Tip('Click here to <b>UPDATE</b> SciMiner identification results to the new ID above.')\" onmouseout=\"UnTip()\"><img src=\"Images/UPDATE.jpg\"></a>
			<a href=\"#\" onclick=\"confirm_delete()\" onmouseover=\"Tip('Click here to <b>DELETE</b> SciMiner identification results according to the selection option.')\" onmouseout=\"UnTip()\"><img src=\"Images/DELETE.jpg\"></a>
			</form>
			";
...
{	my $sen2geneID					= shift;
	...
	my $userID						= shift;
	my $pmid						= shift;
	my $targetSen2GeneIDString		= shift;
	...
	{	#  Check for the remaining file existence
		$sql			= "SELECT sen2geneID, geneID, hgncID, approvedSymbol, matchString, actualString, inExClude, inExCludeCond FROM sentence2gene where sen2geneID in ($targetSen2GeneIDString) and hgncID = $oldHUGOID";
		$sth         	= $dbh->prepare($sql);
		$sth->execute();
		while(@row = $sth->fetchrow_array)
		{	# Check for matching condition
			if (($row[2] eq $oldHUGOID) && ($row[4] eq $matchString) && ($row[5] eq $actualString))
			{	#  Not defined
				...
				#  Update the contents
				$dbh->do("UPDATE sentence2gene SET geneID = $newGeneID, hgncID = $newHugoID, approvedSymbol = \"$newApprovedSymbol\", editTag = 1, editUser = $userID, inExClude=1, inExCludeCond=\"UserEdit\", oldGeneID = $row[1], oldHgncID = $row[2], oldApprovedSymbol = \"$row[3]\", oldInExClude=$row[6], oldInExCludeCond=\"$row[7]\" WHERE sen2geneID = $row[0]") || LogQuery("! ERROR: Cannot update the sentence2gene for sen2geneID $row[0]");
				$updateCount++;					
			}
		}
	...
	{	#  Check for the remaining file existence
		$sql			= "SELECT sen2geneID, geneID, hgncID, approvedSymbol, matchString, actualString, inExClude, inExCludeCond FROM sentence2gene where sen2geneID in ($targetSen2GeneIDString) and pmid = $pmid and hgncID = $oldHUGOID";
		$sth         	= $dbh->prepare($sql);
		$sth->execute();
		while(@row = $sth->fetchrow_array)
		{	# Check for matching condition
	...
	{	#  Check for the remaining file existence
		...
		{	# Check for matching condition
			...
			{	#  Not defined
				...
				#  Update the contents
				$dbh->do("UPDATE sentence2gene SET inExClude=2, inExCludeCond=\"UserEdit\", editTag = 1, editUser = $userID, oldInExClude=$row[6], oldInExCludeCond=\"$row[7]\" WHERE sen2geneID = $row[0]") || LogQuery("! ERROR: Cannot update the sentence2gene for sen2geneID $row[0]");
				$deleteCount++;					
			}
		}
