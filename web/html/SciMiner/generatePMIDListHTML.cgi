...
{	$pmidListFileName	= "FinalResults/$resultBaseDir/pmidList.html";
	...
	my $newHTMLToBeCreated	= "FinalResults/$currentBaseDir/Gene/pmidList_".$extHugoID.'_'.$typeColumn.'.txt';
	...
	{	#  Load PMIDs to be extracted
		open (LIST, "FinalResults/$currentBaseDir/GeneEnrichmentResult.pmid");
		while (<LIST>)
		{	my $line = $_;
			if ($line =~ /^#/)
			{	next;
