...
)
{
	...
	my $tmpFileName 	= "FinalResults/$fileNameBase/hugoID2ContDir/$hugoID.html";
	if (-f $tmpFileName)
...
{   my $hugoID                      = shift;
    ...
    my $cssURL						= 'mm_health_nutr.css';
    my $EDITIcon					= 'Images/EDIT.jpg';
	...
	$sth	= $dbh->prepare("SELECT * FROM sentence2gene WHERE sen2geneID in ($targetSen2GeneIDString)");
	...
	{	$outputString .= "<tr><td><a href=\"$ncbiPubMedURL$row[1]\">$row[1]</a></td><td>$row[6]</td><td>$row[7]</td><td>$row[9]</td><td>$row[10]</td>";
		...
		#  Edit Button
		$outputString .= "<td><a href=\"javascript:editSciMinerFindingFromMergedFile('$row[0]','$fileNameBase','$row[6]','$row[7]','$hugoID');\"><img src=\"$EDITIcon\"></a></td></tr>\n";
	}
