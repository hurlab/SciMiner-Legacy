...
{	$geneSummaryFileName	= "FinalResults/$resultBaseDir/geneSummary.html";
	
	#  First check the file to be created
	my $newHTMLToBeCreated	= "FinalResults/$currentBaseDir/Pathway/geneSummary_".$pathwayID.'_'.$typeColumn.'.txt';
	...
	{	#  Load PMIDs to be extracted
		...
		{	# Now load the pmidList.html file and recreate a new htmlFild
			open (ORIHTML, "FinalResults/$resultBaseDir/geneSummary.html") || return(-1);
			my @content	= <ORIHTML>;
			close ORIHTML;
			my $conString	= join (" ", @content);
			$conString		=~ s/\r|\n//g;
			
			my $finalResultString	= '<table border="1" width="950">';
				...
				{	my $newMatch	= $&;
					...
					{	if ($newMatch	=~ /hgnc_id=(\d+)/)
						{	if (defined $hugoIDCheck{$1})
							{	$newMatch	=~ s/"pmid2ListDir/"FinalResults\/$resultBaseDir\/pmid2ListDir/;
								$newMatch	=~ s/"hugoID2ContDir/"FinalResults\/$resultBaseDir\/hugoID2ContDir/;
								$finalResultString	.= $newMatch."\n";
							}
						}
					}
